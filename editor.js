/* ═══════════════════════════════════════════════════════════════
   editor.js — Split-pane editor: CodeMirror + live preview,
               frontmatter form, image drag-drop positioning
   ═══════════════════════════════════════════════════════════════ */
...
// ─── CodeMirror CDN loader (loaded once) ───────────────────────
let cmLoaded = false;
...
function mountEditor({ slug, content, isExisting, title }) {
  ...
  document.getElementById('editor-filename').value    = slug;
  document.getElementById('editor-filename').readOnly = isExisting;
  document.getElementById('editor-filename').style.opacity = isExisting ? '0.6' : '';
  document.getElementById('editor-is-existing').value = isExisting ? 'true' : 'false';
  overlay.classList.add('visible');

  // Populate frontmatter form from content
  const { frontmatter } = extractFrontmatter(content);
  populateFmForm(frontmatter);

  // Destroy old CM instance if any
  ...
  // Style the CM wrapper to fill the pane
...
function updatePreview() {
  ...
  try {
    ...
    // Make wiki-links inside preview non-navigating (just style them)
    pane.querySelectorAll('a[onclick]').forEach(a => {
...
// ─── EDITOR TABS (Write / Preview / Frontmatter) ──────────────
export function switchEditorTab(tab) {
  ...
  writePanelWrap.style.display  = (tab === 'write' || isSplit) ? 'flex' : 'none';
  previewPanel.style.display    = (tab === 'preview' || isSplit) ? 'block' : 'none';
  fmPanel.style.display         = tab === 'fm' ? 'block' : 'none';

  document.getElementById('editor-panes').classList.toggle('split-mode', isSplit);
...
function handleImageDrop(e) {
  ...
  if (!text) return;
  // Expect text to be an image filename from the commons picker
  const snippet = buildImageSnippet(text, 'right');
  if (State.cmEditor) {
...
export function insertImageAtCursor(filename, align = 'right') {
  if (!State.cmEditor) return;
  const snippet = buildImageSnippet(filename, align);
...
function buildImageSnippet(filename, align) {
...
function showSaveInstructions(url, content, copied, isEdit) {
  ...
  else        { msgEl.textContent = 'Clipboard access unavailable — copy the content below manually.'; msgEl.style.color = 'var(--muted)'; }
  ...
  document.getElementById('save-instructions-steps').innerHTML = isEdit
    ? `<li>Click the link above to open the file in GitHub's editor.</li>
       <li>Select <strong>all</strong> existing content (<kbd>Ctrl+A</kbd> / <kbd>Cmd+A</kbd>).</li>
...
// ─── COMMONS IMAGE PICKER (inside editor) ────────────────────
export function openImagePicker() {
  document.getElementById('editor-image-picker').classList.add('visible');
  renderImagePicker('');
}
...
function renderImagePicker(q) {
  ...
  grid.innerHTML = hits.slice(0, 60).map(img => {
    ...
    return `<div class="img-picker-item" draggable="true"
        ...
        title="${img.filename}${img.description ? ': ' + img.description : ''}">
  ...
  }).join('') || '<p style="color:var(--muted);padding:16px;">No images found.</p>';
}
