...
import { RAW_BASE, REPO_OWNER, REPO_NAME, BRANCH, IMAGES_PATH } from './state.js';
...
export async function showCommons() {
  ...
  // Store images globally for filter
...
function renderCommonsGrid(images) {
  return images.map(img => {
    ...
    const thumb = url; // thumbnails are same URL for now; build-commons.js can generate thumbs
    return `<div class="commons-card" data-filename="${escapeAttr(img.filename)}" data-desc="${escapeAttr(img.description || '')}">
      <div class="commons-thumb-wrap" onclick="window.__openLightbox('${escapeAttr(url)}','${escapeAttr(img.title || img.filename)}')">
        <img
          ...
          alt="${escapeAttr(img.title || img.filename)}"
          loading="lazy"
...
function buildRefList(images) {
  ...
  return images.map(img =>
    `${img.filename}${img.title ? ' — ' + img.title : ''}${img.description ? '\n  ' + img.description : ''}`
  ).join('\n\n');
}
...
export async function getFeaturedImage() {
  ...
  // Use date-seeded selection so it changes daily
